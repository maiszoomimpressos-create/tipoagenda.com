...
serve(async (req) => {
  ...
  try {
    ...
    // 8. Send notification email to admin about new company registration
    ...
    if (RESEND_API_KEY) {
        try {
            ...
            const formattedCompanyPhone = formatPhone(companyPhoneNumber);
            ...
            console.error('❌ Error sending notification email:', notificationErr.message);
            console.error('Error details:', notificationErr);
    ...
    // 9. Send email confirmation with redirect to plans page
