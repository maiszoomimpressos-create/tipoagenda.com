...
serve(async (req) => {
  ...
  let expectedKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') || '';
  
  // Se encontrou na variável de ambiente, comparar diretamente
  if (expectedKey && expectedKey !== '') {
    if (providedKey !== expectedKey) {
      console.error('❌ Acesso negado: Service role key inválida (não corresponde à variável de ambiente)');
      return new Response(
    ...
    // Se não encontrou na variável de ambiente, verificar se a chave fornecida é válida
    // tentando ler a tabela app_config com ela
    try {
      ...
      // Se conseguiu ler, verificar se a chave fornecida corresponde à chave na tabela
      if (providedKey !== config.value) {
        console.error('❌ Acesso negado: Service role key não corresponde à chave na tabela app_config');
        return new Response(
          JSON.stringify({ error: 'Acesso negado. Service role key inválida.' }),
          {
            status: 403,
      ...
      return new Response(
        JSON.stringify({ error: 'Acesso negado. Erro ao validar service role key.' }),
        {
          status: 403,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        },
  ...
  try {
    ...
    // Usar ISO string em UTC (now.toISOString()) porque scheduled_for é TIMESTAMPTZ
    // O PostgreSQL vai fazer a conversão de timezone automaticamente
    ...
    // Se não encontrou mensagens, tentar query alternativa usando SQL raw
    let finalPendingLogs = pendingLogs || [];
    
    if (!finalPendingLogs || finalPendingLogs.length === 0) {
      console.log(`⚠️ Query padrão não encontrou mensagens. Tentando query alternativa...`);
      
      // Query alternativa: usar SQL raw para garantir comparação correta de timestamp
      const { data: alternativePendingLogs, error: altError } = await supabaseAdmin.rpc('get_pending_messages', {
        p_now: nowISOString,
      }).catch(async () => {
        // Se a função RPC não existir, fazer query manual com todas as mensagens PENDING
        console.log('Função RPC não encontrada, buscando todas as mensagens PENDING...');
        ...
        if (allPendingError) {
          ...
          return { data: null, error: allPendingError };
        }
        ...
        console.log(`Filtradas ${filtered.length} mensagens de ${allPending?.length || 0} total`);
        return { data: filtered, error: null };
      });
      
      if (altError) {
        console.error('Erro na query alternativa:', altError);
      } else if (alternativePendingLogs && alternativePendingLogs.length > 0) {
        console.log(`✅ Query alternativa encontrou ${alternativePendingLogs.length} mensagens`);
        finalPendingLogs = alternativePendingLogs;
      }
    }
