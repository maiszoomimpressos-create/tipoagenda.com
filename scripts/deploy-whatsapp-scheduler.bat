...
if not exist "supabase\functions\whatsapp-message-scheduler\index.ts" (
    echo ERRO: Arquivo nao encontrado!
    echo Procurando: supabase\functions\whatsapp-message-scheduler\index.ts
    pause
    exit /b 1
)

echo [1/3] Copiando codigo para clipboard...
type "supabase\functions\whatsapp-message-scheduler\index.ts" | clip
echo OK! Codigo copiado para clipboard.
echo.

echo [2/3] Abrindo Supabase Dashboard...
start https://supabase.com/dashboard/project/tegyiuktrmcqxkbjxqoc/functions/whatsapp-message-scheduler
echo.
